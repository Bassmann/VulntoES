...
class OpenVasES:
            ...
            for port in p:
                ...
                if port.text != 'general/tcp':
                    ...
                    if port.find('host') is not None:
                        portDict['ip'] = port.find('host').text
                    if d is not None:
    ...
    def toES(self):
        for item in self.issueList:
            self.es.index(index=self.index_name, body=json.dumps(item), doc_type="vuln")
        for port in self.portList:
            self.es.index(index=self.index_name, body=json.dumps(port), doc_type="vuln")
